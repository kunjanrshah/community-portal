...
class GlobalSearch extends REST_Controller {
    ...
    function index() {
        ...
        if (($this->flag) == "1") {
            ...
            if (!empty($request)) {
                $users = $this->model_name->getUsersByGlobalSearch($request, $user_id);
                $totalUsers = $this->model_name->getUsersByGlobalSearchCount($request);
